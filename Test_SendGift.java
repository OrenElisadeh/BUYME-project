...
public class Test_SendGift {
    ...
    @Test
    @Order(1)
    @DisplayName("Testing the gift sending process")
    public void test_SendingGift() throws Exception {
        ...
        currentURL = driver.getCurrentUrl();
        logger.info("Filling sendToWhoPage fields");
        sendtowhopage.fillSendToWhoFields();

        //howToSend Page
        basepage.assertingPagesByURL("howToSendPage", currentURL);
        currentURL = driver.getCurrentUrl();
        logger.info("Filling howToSendPage fields");
        howtosendpage.fillHowToSendFields();
        logger.info("Asserting and taking payment page snap shot");
        basepage.assertingPagesByURL("paymentPage", currentURL);
    }
